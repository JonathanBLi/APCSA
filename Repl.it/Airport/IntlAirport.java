/** The IntlAirport class is a collection of international airports. It provides methods to update the number of gates at an airport and get a list of IATA codes.
 * 
 * @author Jonathan Li
 * Collaborators: None
 * Teacher: Bailey
...
public class IntlAirport
{
  ...
  {
    for (Airport a : intlAirportList)
    {
      if (a.getAirportCode().equals(aCode))
      {
        a = new Airport(a.getAirportCode(), adjustedGates, true);
        return a;
      }
    }
    return null;
  ...
  {
    String[] iata = new String[intlAirportList.length];
    for (int i = 0; i < intlAirportList.length; i++)
    {
      iata[i] = intlAirportList[i].getAirportCode().substring(2);
    }
    return iata;
  }
